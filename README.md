# 3-Stage NMOS Differential Amplifier Design

[![Domain](https://img.shields.io/badge/Domain-Analog%20IC%20Design-orange.svg)](#)
[![Tools](https://img.shields.io/badge/Validation-PSPICE-blue.svg)](#)

## Overview
This repository showcases the theoretical design and simulation verification of a 3-stage, fully differential NMOS amplifier. The project demonstrates a rigorous mixed-signal design methodology: establishing strict theoretical bias points and small-signal parameters via hand-calculation, followed by schematic capture and AC/DC simulation verification. 

The architecture is designed to meet strict gain, output resistance, and precise frequency roll-off characteristics (poles/zeros) required for sensor interfacing and precision mixed-signal applications.
...
* **Biasing Strategy:** Three independent current mirrors ($M_{9,10}$, $M_{7,8}$, $M_{3,4}$) act as ideal tail current sources. Symmetrical design ensures uniform drain current ($I_D$) splitting across the differential pairs, heavily stabilizing the DC operating points.
...
*(Detailed derivations for $g_m$, bias point KVLs, and saturation validations can be found in the accompanying project report).*

## Simulation Results

### AC Sweep (Bode Plot)
*The simulation confirms the 36.17 dB peak gain and the strict cutoff frequencies.*
![AC Sweep Plot](images/ac_sweep.png)

### DC Bias Point Schematic
*Annotated schematic verifying the calculated gate-source voltages and tail currents.*
![Schematic](images/schematic_bias.png)
